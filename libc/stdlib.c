...
#define TEST_GRANULE_SIZE(i) \
  if (granules <= i) return GRANULES_##i;
...
static char *allocate_chunk(struct page *page, unsigned idx,
...
static struct large_object **maybe_merge_free_large_object(
    struct large_object **prev) {
  ...
  *best_prev = next;
  ...
  if (tail_size) {
    ...
    if (start_page == get_page(end - tail_size - 1)) {
      ...
      char *ptr =
          allocate_chunk(next_page, FIRST_ALLOCATABLE_CHUNK, LARGE_OBJECT);
      best = (struct large_object *)ptr;
      best->size = best_size =
...
__LIBC_INLINE size_t size_to_granules(size_t size) {
...
__LIBC_EXPORTS void *malloc(size_t size) {
